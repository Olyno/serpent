...
Affiche la documentation et les signatures des symbols
...
def _get_node_documentation(node: BaseNode) -> str:
    """
    Generate a documentation string for an AST node.
...
def _format_function_doc(func: nodes.FunctionDef) -> str:
    """Format the documentation for a function."""
...
def _format_variable_doc(var: nodes.VariableDecl) -> str:
    ...
    name = var.target.id if var.target else "unknown"
